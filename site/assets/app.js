(function () {
  'use strict';
  const C = Augur, W = AugurWorkflows, profiles = AUGUR_PROFILES;
  ...
  const steps = [
    ['Plan','What would you like to build?','Start with the outcome. AUGUR prepares the configuration, the commands and the documentation together.'],
  ...
  function guide(section='guides') {
    ...
    modal('AUGUR field guide',`<p>Inspected AutoUpgrade ${esc(project.profileId)}. Configuration, instructions and sources stay available offline.</p>${tabs}${body}<h3>Further reading</h3>${sourceLinks(W.SOURCES.map(s=>s.id))}`);
  }
  function render() {
  ...
  document.addEventListener('click',event=>{
    ...
    if(el.id==='download-runbook')saveBlob(W.markdown(project,profile()),'augur-runbook.md','text/markdown;charset=utf-8');
